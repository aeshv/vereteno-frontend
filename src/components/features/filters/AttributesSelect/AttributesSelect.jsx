...
export function AttributesSelect() {
    ...
    const onAttributesChange = (e, name = '') => {
        ...
        setValue((prevState) => [...prevState.filter((item) => item.name !== res.name), res])
    };

    useEffect(() => {
        const temp1 = [].concat(value?.map((item) => {
            return (
                item.values
            )
        }))
        router.query.attributes = Array.from(new Set(temp1.reduce((acc, current) => {
    ...
    return (
        <Accordion multiple={true} classNames={classes}
                   className={classes.root}>
            {tempData?.map((singleAttribute) => {

                return (
                    <>
                        <Accordion.Item key={singleAttribute.id} value={singleAttribute.name}>
                            <Accordion.Control
                                disabled={!singleAttribute?.values?.length}>{singleAttribute.name}</Accordion.Control>
                            <Accordion.Panel>
                                <AttributeItem  {...singleAttribute} onChange={onAttributesChange}/>
                            </Accordion.Panel>
                        </Accordion.Item>
                    </>
                )
