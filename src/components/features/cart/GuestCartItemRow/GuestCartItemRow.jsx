...
const GuestCartItemRow = ({item, isSelected, toggleRow, isDisabled}) => {
    ...
    const currentItemVendorCode = item?.vendorCodes.find((vendor) => vendor.productVendorCodeId === item?.productVendorCodeIds)
    ...
    return (<tr key={item.id} className={cx({[classes.rowSelected]: isSelected})}>
        ...
        <td>
            <QuantityInput disabled={isSelected} current={item.quantity || 1} handleChange={() => {
            }}/>
        </td>
        <td>{item?.discount?.discount_coefficient ?
            // Со скидкой
            <Stack spacing={'xs'}>
                d руб.
                <Badge variant="gradient" gradient={{
                    from: theme.colors.brand[4], to: theme.colors.brand[8]
                }}>Скидка {100 - item?.discount?.discount_coefficient * 100}%</Badge>
            </Stack>
            :
            // Без скидки
            <Stack spacing="xs">
                {currentItemVendorCode?.price * item?.quantity !== currentItemVendorCode?.price &&
                    <Text>1 шт. - <b>{currentItemVendorCode?.price}</b> руб.</Text>
                }
                <Text>Всего - <b>{currentItemVendorCode?.price * item?.quantity}</b> руб.</Text>
            </Stack>}
        </td>
        <td>
            <Group spacing={0} position="center">
                <ActionIcon variant="light" color="red" loading={false}
                            onClick={() => {
                            }}>
